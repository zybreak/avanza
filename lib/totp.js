 ...
 * @returns {string}
 */
export default (secret, timeStep = 30, numDigits = 6) => {
  const alg = 'sha1';
  const counter = Math.floor((Date.now() / 1000) / timeStep);

  // If secret is not already a buffer, treat is as base32 encoded string.
  secret = Buffer.isBuffer(secret) ? secret : Buffer.from(secret.split('')
        .filter((x) => {
          const y = x.charCodeAt(0);
          return (y >= 0x32 && y <= 0x37) || // 2-7
                (y >= 0x41 && y <= 0x5B) || // A-Z
                (y >= 0x61 && y <= 0x7B) // a-z
        })
        .map((x) => {
  ...
  const counterBuf = Buffer.from(
    [24, 16, 8, 0].map(x => ((counter / (2 ** 32)) >> x) & 255).concat(
      [24, 16, 8, 0].map(x => ((counter % (2 ** 32)) >> x) & 255)
    )
  );
